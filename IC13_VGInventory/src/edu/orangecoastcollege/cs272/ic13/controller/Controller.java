...
public class Controller {
	...
	public String signUpUser(String name, String email, String password)
	{
	    ...
	    if (!isValidEmail(email))
	        return "Email address not valid. Please try a different address.";

//	    if (!isValidPassword(password))
//	        return "Password must be at least 8 characters, including 1 upper case letter,\n1 lower case letter, 1 symbol";
        ...
        {
	        int id =
                   theOne.mUserDB.createRecord(Arrays.copyOfRange(USER_FIELD_NAMES, 1, USER_FIELD_NAMES.length), values);
	        theOne.mCurrentUser = new User(id, name, email, "user");
	        theOne.mAllUsersList.add(theOne.mCurrentUser);
        }
        catch (SQLException e)
        {
            // TODO Auto-generated catch bloc9k
            e.printStackTrace();
	...
	public ObservableList<VideoGame> getGamesForCurrentUser()
	{
		...
		try
        {
            ArrayList<ArrayList<String>> resultsList = theOne.mUserGamesDB.getRecord(Integer.toString(theOne.mCurrentUser.getId()));
            // Loop through the results
            int gameId;
            for (ArrayList<String> values : resultsList)
            {
                gameId = Integer.parseInt(values.get(1));
                // Loop through all the games, and try to find a match
	...
	public boolean addGameToUsersInventory(VideoGame selectedGame)  {
	    ...
	    // Create an ObservableList<VideoGame> assigned to the list returned from getGamesForCurrentUser
